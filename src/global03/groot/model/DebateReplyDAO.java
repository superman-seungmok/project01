...
public class DebateReplyDAO {
	private static DebateReplyDAO instance = new DebateReplyDAO();
	private DebateReplyDAO() {};
	public static DebateReplyDAO getInstance() { return instance; }
	...
	public Integer getCnt(String id) {
		...
		try {
			conn = getConn();
			String sql = "select count(*) from debateReply where id=?";
	...
	public Integer getCnt(String currId, String sel, String search) {
		...
		try {
			conn = getConn();
			String sql = "select count(*) from debateReply where id=? and "+sel+" like '%"+search+"%'";
			pstmt = conn.prepareStatement(sql);
	...
	public List getReplies(String currId, Integer startRow, Integer endRow) {
		...
		try {
			...
			String sql = "SELECT b.* FROM" + 
					...
					"(SELECT * FROM debateReply WHERE id=? "
			...
			if(rs.next()) {
				...
				do {
					DebateReplyDTO dbrDto = new DebateReplyDTO();
					dbrDto.setPostNo(rs.getInt("postNo"));
					dbrDto.setId(rs.getString("id"));
					dbrDto.setContent(rs.getString("content"));
					dbrDto.setPostNo(rs.getInt("postNo"));
					dbrDto.setRef(rs.getInt("ref"));
					dbrDto.setRe_level(rs.getInt("re_level"));
					dbrDto.setLiked(rs.getInt("liked"));
					dbrDto.setReg(rs.getTimestamp("reg"));
					
					articleList.add(dbrDto);
				}while(rs.next());
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	public List getSearchReplies(String currId, Integer startRow, Integer endRow, String sel, String search) {
		...
		try {
			...
			String sql = "SELECT b.* FROM" + 
					"(SELECT a.* , rownum r FROM" + 
					"(SELECT * FROM debateReply WHERE id=? and "+sel+" like '%"+search+"%' "
					+ "ORDER BY reg DESC) a ORDER BY reg DESC) b WHERE r>=? AND r<=?";
			pstmt = conn.prepareStatement(sql);
	...
	public int getReplyCount(int postNo) {
		...
		try {
			conn = getConn();
			String sql = "select count(*) from debateReply where postNo=?";
			pstmt = conn.prepareStatement(sql);
	...
	public List getReplies(int postNo, int start, int end) {
		...
		try {
			conn = getConn();
			String sql = "SELECT b.*, r FROM (SELECT a.*, rownum r FROM (SELECT * FROM debateReply WHERE postNo=? "
			...
			if(rs.next()) {
				...
				do {
					DebateReplyDTO reply = new DebateReplyDTO();
	...
	//게시글 번호로 해당 글의 댓글 수를 리턴하는 메서드[윤서]
	public int returnReplyCount(int postNo) {
		int x = 0;
		try {
			conn = getConn();
			String sql = "select count(*) from debateReply where postNo=?";
	...
	//자유토론 댓글 입력(저장)하는 메서드 [윤서]
	public void insertReply(DebateReplyDTO reply) {
		int number = 0;
		...
		try {
			conn = getConn();
			String sql = "select max(replyNo) from debateReply";
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()) {
			...
			sql = "insert into debateReply values(debateReply_seq.nextVal,?,?,?,?,?,?,?,sysdate)";
			pstmt = conn.prepareStatement(sql);
